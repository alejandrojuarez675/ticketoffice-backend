...
public class EventDetailPageHandler {
    ...
    private Function<Ticket, TicketDTO> getTicketPricePriceDTOFunction(Event event) {
        return price -> {
            try {
                Integer availableTicketStock = getAvailableTicketStockIdUseCase.apply(event, price.id());
                return new TicketDTO(price.id(), price.value(), price.currency(), price.type(), price.isFree(), availableTicketStock);
